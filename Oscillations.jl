...
begin
	abstract type physObject end
		
	mutable struct Ball <: physObject
		pos::ComplexF64
		vel::ComplexF64
		force::ComplexF64
	...
	Ball(p::ComplexF64, m::Float64, r::Float64, isAnchor::Bool = false) = Ball(p, 0 + 0im, 0 + 0im, m, r, isAnchor)
	...
	function applyForce(s::Spring)
		l = (s.e1.pos - s.e2.pos)
		F = s.k * (abs(l) - s.l0) * l/abs(l)
...
let
	...
	function circ(p = O, alph = 1.0, action = :fill, radius = 25, edge = "solid", isAnchor = false)
	    ...
	    if(!isAnchor)
			setcolor(0., 0., 1.0, alph)
			circle(p, radius, action)
		else
			setcolor(0., 1.0, 0., alph)
			circle(p, radius/3, action)
		end
