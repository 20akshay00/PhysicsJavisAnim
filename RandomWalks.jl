...
# ╔═╡ 03257750-42df-11ec-0e39-f16f40d295c9
begin
	using Javis, Animations, StatsBase
end

# ╔═╡ da610f95-c5bd-49ce-a199-d7896b635b08
function randomWalk(numsteps = 100)
	moves = [Point(0, 1), Point(0, -1), Point(1, 0), Point(-1, 0)]
	
	traj = accumulate(sample(moves, numsteps - 1), init = Point(0, 0)) do old, move
		old + move
	end
	
	return vcat(Point(0, 0), traj)
end

# ╔═╡ 46351f65-5d77-46c5-9f56-bbe4064f6f17
md"""
## Single particle
"""

# ╔═╡ 12cb472f-f9e6-4031-bc67-da0aa2f33a05
let
	...
	draw_line.(points[2:end], points[1:end-1], color)
end
	...
	r = 5
	bpath = randomWalk(cut_off)
	path_of_ball = Point[]
	
	banim = Animation(
				range(0, stop = 1, length = cut_off),
			    10*bpath,
				linear())
	
	ball = Object(1:nframes, (args...) -> circ(O, [0.0, 0.0, 0.7], 0.9, :fill, r), Point(0, 0))
	act!(ball, Action(1:cut_off, banim, translate()))
	
	ball_traj = Object(1:cut_off + fade_time, (args...)->path!(path_of_ball, pos(ball), "blue"))
	act!(ball_traj, Action(cut_off: cut_off + fade_time, disappear(:fade)))
...
# ╔═╡ 5d1c1e58-8693-4d7b-83f1-36e37aa2e7f3
md"""
## Multiple particles
"""

# ╔═╡ 585605d9-d895-43c4-b037-5b25e4e01793
let
	function ground(args...)
	    background("black")
	    sethue("white")
	...
	r = 1.5
	num_walkers = 10
	bpath = [randomWalk(cut_off) for i in 1:num_walkers]
	path_of_ball = [Point[] for i in 1:num_walkers]
	ball_traj = []
	...
	ball = [Object(1:nframes, (args...) -> circ(O, [0.0, 0.0, 0.7], 0.9, :fill, r), Point(0, 0)) for i in 1:num_walkers]
	
	for i in 1:num_walkers
		act!(ball[i], Action(1:cut_off, banim[i], translate()))
		push!(ball_traj, Object(1:cut_off + fade_time, (args...)->path!(path_of_ball[i], pos(ball[i]), "blue")))
		
		act!(ball_traj[i], Action(cut_off: cut_off + fade_time, disappear(:fade)))
	end
